...
public class SolverSpawner extends SatinObject implements Serializable,
    ...
    public boolean guard_spawn_recSearch(Solver solver, long nofConflicts,
        int satinDepth, SolverState globalState, Vec<VecInt> reasons) {
        return (globalState.iter == solver.satinSearchIter);
    }

    public SolverResult spawn_recSearch(Solver solver, long nofConflicts,
        int satinDepth, SolverState globalState, Vec<VecInt> reasons) {
        SolverResult res;

        res = recSearch(solver, nofConflicts, satinDepth, globalState, reasons);
        return res;
    }

    public SolverResult recSearch(Solver solver, long nofConflicts,
        int satinDepth, SolverState globalState, Vec<VecInt> reasons) {
        SolverResult res;
        ...
        res = solver.satinDoRecSearch(nofConflicts, satinDepth, globalState,
            reasons);
        if (!res.seqTimeOut) {
            // Done
            return res;
        ...
        if (solver.satinUseSharedObjects) {
            ...
            child_nofConflicts = (nofConflicts - solver.lastConflicts) / 16 * 15;
        }
