...
public class Dsearch_AlgorithmV1
{
	...
	{
        Vector querySequencesPointers    = (Vector)workUnit.get(3);
        Vector databaseSequencesPointers = (Vector)workUnit.get(4);
        ...
        for(int i = 0; i < querySequencesPointers.size(); i++)
        {
            Sequence querySequence;
            String querySequenceName;
            String querySequenceBody;

            querySequence = new Sequence((Sequence)querySequencesPointers.get(i));
            querySequence.createSequenceName();
            querySequence.createSequenceBody();

            querySequenceName = querySequence.getSequenceName();
            querySequenceBody = querySequence.getSequenceBody();
            ...
            for(int j = 0; j < databaseSequencesPointers.size(); j++)
            {
                Sequence databaseSequence;
                String databaseSequenceName;
                String databaseSequenceBody;

                databaseSequence = new Sequence((Sequence)databaseSequencesPointers.get(j));
                databaseSequence.createSequenceName();
                databaseSequence.createSequenceBody();

                databaseSequenceName = databaseSequence.getSequenceName();
                databaseSequenceBody = databaseSequence.getSequenceBody();
