...
public class InputReader  implements AlignmentAlgorithms
{
	...
	public InputReader() throws Throwable
	{
		...
		BufferedReader inputs = new BufferedReader(new FileReader(new File(PROBLEMDIRECTORY, PARAMETER_FILE)));
		...
		if(property == null)
		{
			throw new Exception("Could not get name of database.file parameter from parameter file");
		}

		database = new File(PROBLEMDIRECTORY, property);
		if(! database.isFile())
		{
			throw new Exception("Cannot find database file - was it included as problem data? Was the name of the database file entered into the parameter file");
		}

		property = parameters.getProperty("query.file");
		if(property == null)
		{
			throw new Exception("Could not get name of query.file parameter from parameter file");
		}

		query = new File(PROBLEMDIRECTORY, property);
		if(! query.isFile())
		{
			throw new Exception("Cannot find query file - was it included as problem data? Was the name of the query file entered into the parameter file?");
		}

        property = parameters.getProperty("scores.or.alignments");
        if(property == null)
            throw new Exception("Could not get scoresOrAlignments parameter from parameter file");
		...
		{
			throw new Exception("Check scoresOrAlignments parameter in parameter file");
		}

        property = parameters.getProperty("top.scores");
		if(property == null)
		{
			throw new Exception("Could not get top.scores parameter from parameter file");
		}
        ...
        if(property == null)
        {
            throw new Exception("Could not get value.threshold parameter from parameter file");
        }
        ...
        {
            throw new Exception("Check value.threshold parameter in parameter file");
        }
		...
		if(! alignmentAlgorithm.equals(NEEDLEMAN_WUNSCH) && ! alignmentAlgorithm.equals(SMITH_WATERMAN) && ! alignmentAlgorithm.equals(CROCHEMORE_GLOBAL) && ! alignmentAlgorithm.equals(CROCHEMORE_LOCAL))
		{
			throw new Exception("Alignment algorithm must be: " + NEEDLEMAN_WUNSCH + " or " + SMITH_WATERMAN + " or " + CROCHEMORE_GLOBAL + " or " + CROCHEMORE_LOCAL);
		}

		property = parameters.getProperty("scoring.scheme");
		if(property == null)
		{
			throw new Exception("Could not get score.scheme parameter from parameter file");
		}
		...
		else if(property.equals("sm"))
		{
			...
			if(property == null)
			{
				throw new Exception("Could not get score.matrix parameter from parameter file");
			}
			File scoringMatrix = new File(PROBLEMDIRECTORY, property);
			if(! scoringMatrix.isFile())
			{
				throw new Exception("Check that the score matrix file was included as problem data");
			}
