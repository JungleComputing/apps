...
/* strictfp */final class BarnesHut extends SatinObject implements
    ...
    static Ibis mcibis;         // Ibis instance used for high throughput multicast

    static MCThread mc;
    
    static BodiesSO bodies = new BodiesSO();
    static ClusterInfo clusterInfo;
    static ClusterMCThread[] clusterThreads;
    static BodyUpdates result;
    ...
    static {
        ...
        IbisIdentifier[] list = mcibis.registry().joinedIbises();
        clusterInfo = new ClusterInfo(list, mcibis.identifier());

        mc = new MCThread(mcibis, list);
        Updater u = new Updater(list.length);
        clusterThreads = new ClusterMCThread[clusterInfo.numClusters()];
        for (int i = 0; i < clusterThreads.length; i++) {
            clusterThreads[i] = new ClusterMCThread(clusterInfo, mcibis, i, list, u); 
        }
    }
...
//    private static int iteration_printed = -2;

    public int doBarnesSO(int numBodies, int nodeId, int iteration) {
...
//        if (iteration != iteration_printed) {
//            System.out.println("Job for iteration " + iteration);
//            iteration_printed = iteration;
//        }
        ...
        if (me.children == null || me.bodyCount < params.THRESHOLD) {
            ...
            me.barnesSequential(root, result, params);
            return 0;
        }
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (ch != null) {
                ...
                int v = /* spawn */BarnesSO(ch.bodyCount, ch.getId(), iteration);
                childcount++;
            }
        }
    ...
    void runSim() {
        ...
        for (int iteration = 0; iteration < params.ITERATIONS; iteration++) {
            ...
            doBarnesSO(bodyArray.length, 0, iteration);
            ...
            try {
                mc.multicastIteration(iteration);
            } catch (IOException e) {
                System.out.println("Oops: got exception in multicastIteration");
                e.printStackTrace();
                System.exit(1);
            }
            
            bodies.waitForRoot(iteration);
